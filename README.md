...
The raw datasets used in this project were obtained via manual download from Kaggle due to access restrictions and licensing requirements. As a result, data acquisition is not automated through a script. Users reproducing this project should manually download the datasets from their original Kaggle sources and place them into the `data/raw/` directory before running any scripts. Because the data is manually downloaded, cryptographic checksums (e.g., SHA-256) were not generated as part of this workflow.
...
Then we took a closer look at the scatterplot which revealed several interesting patterns. We noticed that high-revenue outliers stand out clearly and tend to cluster around ratings between 6 and 8, which reflects the reality that many large-scale franchise films do not rely on exceptional critical reception to succeed financially. In contrast, movies with very high IMDb ratings (above 8) rarely achieve comparable revenue, suggesting that critical acclaim and financial success are not entirely correlated. There also appears to be a dense cluster of mid-budget films centered around ratings of 6–7 with devent revenue levels, representing the majority of releases. These patterns underscore that while ratings correlate weakly with revenue, audience size, marketing resources, and franchise recognition often play a larger role in driving financial outcomes than pure user sentiment.
...
Taken together, these findings highlight the complexity of predicting box office performance using metadata alone. Revenue is shaped not only by a film’s characteristics but also by broader industry trends, seasonal release strategies, franchise effects, and global market expansion. High ratings and popular genres contribute to financial success but do not guarantee it. The variability across years emphasizes the influence of external factors such as major franchise releases or shifts in audience preferences that are not captured directly in our dataset. Ultimately, the everchanging nature of movie performance suggests that understanding box office outcomes requires examining a combination of creative, economic, and temporal features rather than relying on any single variable. These insights provide a foundation for the more advanced modeling approaches described in the Future Work section.


## Future Work

There are several different ways this project could be expanded in the future, and many of them would make the analysis both more accurate and more relevant to how movies actually perform today. One of the most important improvements would be adjusting box office revenue for inflation. Right now, all of the values in our dataset are nominal, which means the numbers do not reflect changes in ticket prices or overall economic conditions. Because of this, a movie from 2005 and a movie from 2023 are technically being compared using dollars that are not worth the same amount. Adjusting for inflation would make comparisons across years more meaningful and would help separate true performance trends from simple changes in the value of money.

It would also be useful to look more closely at how the film industry has changed over time. A lot has shifted in the last two decades. Streaming platforms have become extremely influential, international box office markets have grown, and unexpected events like the COVID-19 pandemic disrupted traditional release patterns. For example, in 2020 and 2021, the number of theatrical releases dropped sharply, and many movies were released online first. Including variables that capture these real-world changes, for instance release strategy or basic economic conditions, could help identify larger patterns that do not appear when comparing yearly medians. Using time-series techniques might reveal long-term trends or turning points that are easy to miss in a standard cross-sectional analysis.

Another area for improvement involves modeling. Most of our current results come from visualization, which is helpful but limited. A predictive model based on features like budget, runtime, genre, and IMDb ratings would allow us to better understand how different variables relate to revenue. More flexible machine learning models, such as random forests or gradient boosting, could capture relationships that are not linear and could handle correlated variables more effectively. Including evaluation steps like train and test splits or cross-validation would also make the results more reliable. It might even be interesting to expand the feature set by incorporating information from movie posters or plot summaries (for example through basic text analysis) to see whether marketing style or narrative themes predict financial performance.

The dataset itself could also be expanded. Many modern films earn a large share of their revenue on streaming platforms instead of in theaters, so adding streaming viewership data would help reflect how the industry functions today. More detailed genre information, franchise connections, or measures of star power could also improve the analysis. Variables related to actors, directors, or studio history may help explain why some movies perform better than others, even when their ratings or budgets look similar on paper.

Finally, combining additional metadata sources through APIs such as TMDb or OMDb would make the dataset much more complete. These sources provide information about cast, directors, production budgets, and marketing spend, all of which are known to influence box office results. Improving the workflow would also be valuable. Clear documentation, along with more automation in the cleaning and merging steps, would make it easier for someone else to repeat or extend the project. Together, these extensions would make the analysis stronger and more capable of addressing how movie performance has changed in an industry that continues to evolve quickly.


## Reproducing

This project uses a Run-All Python script rather than Snakemake for workflow automation and reproducibility
